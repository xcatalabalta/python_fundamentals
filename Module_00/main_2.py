...
def load_exercise(name, folder):
    """
    Load <folder>/<name>.py as a module, without touching sys.path.
    """
    path = os.path.join(BASE_DIR, folder, f"{name}.py")

    spec = importlib.util.spec_from_file_location(name, path)   # step 3
    if spec is None:
        raise ImportError(f"Cannot build a spec for {path}")

    module = importlib.util.module_from_spec(spec)              # step 4
    sys.modules[name] = module                                  # step 5
    spec.loader.exec_module(module)                             # step 6
    return module
...
def test_ft_exercise(exercise_file_name):
    ...
    try:
        ...
        # import sys
        folder = EXERCISES[exercise_file_name]
        # sys.path.append(".")
        ft_module = load_exercise(exercise_file_name, folder)
